...
class TodosTool(
    ...
    override val definition =
        ToolDefinition(
            ...
            description =
                "Manage todos. Actions: list, add, update, complete, cancel, remove. " +
                    "Input: {\"action\":\"list|add|update|complete|cancel|remove\", ...}.",
            inputSchema = STRING_SCHEMA,
        )
...
private fun JsonObject.requiredString(key: String): String = string(key) ?: throw IllegalArgumentException("Missing required field '$key'")
...
private fun resolveWorkspacePathOrFailure(
    toolId: String,
    path: String,
): PathResolution =
