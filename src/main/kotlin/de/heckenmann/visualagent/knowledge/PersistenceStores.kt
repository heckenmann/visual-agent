...
typealias KnowledgeDb = PersistenceStores
...
interface ConversationStore {
    ...
    fun getConversationMessages(sessionId: String, limit: Int = 500): List<ConversationRecord>

    fun getConversationMessagesPage(sessionId: String, limit: Int, offset: Int): List<ConversationRecord>

    fun searchConversationMessages(sessionId: String, query: String, limit: Int = 20): List<ConversationRecord>
...
internal class JpaMemoryStore(
    ...
    @Transactional
    override fun saveMemory(content: String, tags: List<String>): String {
    ...
    override fun saveStructuredKnowledge(subject: String, summary: String, nextSteps: String?): String =
        saveMemory(Json.encodeToString(StructuredKnowledge(subject, summary, nextSteps)), listOf(subject))

    @Transactional(readOnly = true)
    override fun searchMemories(query: String, limit: Int): List<Memory> =
        repository.search(query, PageRequest.of(0, limit.coerceAtLeast(1))).map(MemoryEntity::toDomain)
}
...
internal class JpaConversationStore(
    ...
    @Transactional(readOnly = true)
    override fun getConversationMessages(sessionId: String, limit: Int): List<ConversationRecord> =
        repository
    ...
    override fun searchConversationMessages(
        ...
        return runCatching { repository.searchFts(sessionId, normalized, safeLimit) }
...
internal class JpaSubAgentStore(
    ...
    override fun listAgents(status: String?): List<PersistedSubAgent> =
        (status?.let(repository::findByStatusOrderByCreatedAtDescIdDesc)
            ?: repository.findAllByOrderByCreatedAtDescIdDesc()).map(SubAgentEntity::toRecord)
...
internal class JpaSubAgentConfigStore(
    ...
    @Transactional(readOnly = true)
    override fun listSubAgentConfigs(): List<SubAgentToolConfig> =
        repository.findAllByOrderByIdAsc().map(SubAgentConfigEntity::toDomain)
}
...
private fun ConversationEntity.toRecord(): ConversationRecord =
    ConversationRecord(id, role, content, metadata, createdAt)
