...
4. A conflated work signal makes the coordinator select pending todos by `position` while compatible agents are idle.
   ...
   - Candidate selection scans the backlog once and evaluates each idle agent's execution eligibility once per pickup pass.
...
- Editing a running todo restarts its worker without exposing that agent as idle during the handoff.
