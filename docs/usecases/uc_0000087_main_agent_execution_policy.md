...
3. The main agent uses every tool it has directly where appropriate, including `workspace:file`; it creates a todo only when delegation or long-running work is beneficial.
...
- The system prompt explicitly lists the main agent's available tools and their authorization constraints.
