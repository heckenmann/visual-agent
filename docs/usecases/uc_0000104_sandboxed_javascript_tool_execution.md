...
8. The server enforces time, result, call-count, concurrency, and recursion limits, then closes the context. Script source length is not capped; the normal tool-call timeout remains the execution bound.
