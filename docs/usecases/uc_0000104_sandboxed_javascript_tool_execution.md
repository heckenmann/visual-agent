...
- The current request explicitly enables the `javascript_execute` provider function.
...
1. The agent calls `javascript_execute` with inline `source` or a workspace-relative JavaScript `path`.
...
3. The context receives only `tools.call(name, arguments)`, `tools.list()`, `tools.describe(name)`, the hardened `workspace.write/read/delete(...)` helpers, and bounded simulated `console` methods. Nested tools are identified only by their provider function names; internal IDs are not exposed to scripts.
...
8. The server enforces time, guest-memory (when the Graal isolate runtime is available), result, call-count, concurrency, workspace read/write, tool-argument, and recursion limits, then closes the context. Inline script source length is not capped; workspace-backed source is read through the bounded workspace API, and the normal tool-call timeout remains the execution bound.

## Tool Calls

- `javascript_execute` with `{ "source": "...", "timeoutSeconds": 120 }` or `{ "path": "scripts/report.js" }`; `timeoutSeconds` is optional, accepts 1 through 600 seconds, and uses the application default when omitted.
- Script-internal `tools.call("<enabled provider function name>", { ... })` calls through the existing registry. For example, `workspace_file` calls the tool whose persisted internal ID is `workspace:file`.
...
```javascript
const workspace = await tools.call("workspace_file", { action: "list" });
...
- A script may call only tools enabled in the current request; `javascript_execute` cannot call itself recursively.
