...
9. The configured provider sends the request to the selected backend.
10. The assistant response passes through provider-neutral response normalization, is rendered in the conversation, and is then persisted. Standard chat-protocol framing markers are not shown or reused as dialogue content. If the provider cannot complete the request, a safe, actionable failure message is rendered instead.
