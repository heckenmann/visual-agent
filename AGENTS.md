...
- **Persistence**: `knowledge/PersistenceStores.kt` defines domain `data class`es + `*Store` interfaces. `knowledge/PersistenceEntities.kt` holds the `@Entity internal class`es. `knowledge/JpaPersistenceStores.kt` + `JpaWorkspaceFileStore.kt` adapt Spring Data repositories to the domain interfaces. `KnowledgePersistenceConfig` creates the SQLite `DataSource` (Hikari, maxPool=1, WAL, `busy_timeout=5000`). `application/src/main/resources/db/migration/` holds the tracked schema migrations, including V14 for searchable model skills and its FTS5 index. `application/src/main/resources/config/app.properties` is bootstrap-only and should contain only `database.path`; runtime config is in SQLite `user_preferences`. Conversation and skill search use SQLite FTS5 with a `LIKE` fallback for invalid FTS input.
