...
- `gradle.properties` sets `org.gradle.daemon=false`.
...
- When creating a PR via `gh pr create`, the source branch must be set to auto-delete on merge (`--delete-branch-on-merge` or the `delete_branch_on_merge: true` repo setting).
...
- **Tool inventory** (canonical IDs): `ui`, `history`, `todos`, `context`, `pwd`, `manual`, `usecases`, `file:read`, `file:list`, `file:glob`, `file:grep`, `file:write`, `file:edit`, `terminal`, `sleep`, `browser` (placeholder, returns "not configured"), `search` (placeholder, returns "not configured"), `workspace:layout`, `workspace:file`, `canvas`, `agent:list`, `agent:create`, `agent:update`, `agent:delete`. The main agent only gets the `agent:*` definition tools; sub-agents get role-based sets from `AgentToolConfigService` (default: `researcher`, `coder`, `analyst`). `tools.disabled.global` (preference) is a newline-separated blocklist applied to all agents.
