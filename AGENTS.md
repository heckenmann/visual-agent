...
- **Conversation turn structure**: persist each provider assistant turn independently. A tool-call row must carry an explicit foreign-key parent assistant-turn ID and provider declaration order; never infer ownership from adjacent rows or metadata JSON. Streaming deltas, retry replay, history paging, and local/remote protocol DTOs must preserve assistant-turn identity. Legacy rows without a parent ID remain ungrouped.
