...
class RequestContextBudgeterTest {
    ...
    fun `regular tool schemas may use capacity after the latest user and ten priority answers`() {
        val request =
            ChatRequestContext(
                ...
                parameters = ModelParameters(maxTokens = 20),
                contextWindow = ContextWindow(configuredLimit = 350),
            )
        ...
        assertTrue(plan.request.messages.any { it.content == "older question" })
        assertFalse(plan.status.historyReduced)
    ...
    fun `prioritizes the latest ten assistant answers before lower-priority history`() {
        ...
        val request =
            ChatRequestContext(
                messages,
                parameters = ModelParameters(maxTokens = 20),
                contextWindow = ContextWindow(configuredLimit = 600),
            )
        ...
        assertTrue((2..11).all { index -> result.messages.any { it.content == "question-$index" } })
    }
    ...
    @Test
    fun `fitted provider prompt sends truncated assistant content instead of restoring the original`() {
        val request =
            ChatRequestContext(
                messages = emptyList(),
                parameters = ModelParameters(maxTokens = 20),
        ...
        val prompt =
            Prompt(
                listOf(
                    SystemMessage("rules"),
                    UserMessage("prior question"),
                    AssistantMessage("A".repeat(500)),
                    UserMessage("latest"),
                ),
            )

        val fitted = budgeter.fitPrompt(request, prompt)
        ...
        assertTrue(assistantContent.length < 500)
        assertTrue(assistantContent.endsWith("[Earlier answer truncated to fit the model context window.]"))
    }

    @Test
    fun `preserves summary classification in message metadata across prompt fitting`() {
        val generousRequest =
            ChatRequestContext(
                messages = emptyList(),
                parameters = ModelParameters(maxTokens = 20),
        ...
        val constrainedRequest = generousRequest.copy(contextWindow = ContextWindow(configuredLimit = 150))
        val summary =
            AssistantMessage
                .builder()
                .content("summary".repeat(100))
                .properties(mapOf(ContextPolicyMetadata.KEY to ConversationContextPolicy.SUMMARY_SOURCE.name))
                .build()
        val prompt =
            Prompt(
                listOf(
        ...
        val firstFit = budgeter.fitPrompt(generousRequest, prompt)
        val firstFitSummary = firstFit.instructions.filterIsInstance<AssistantMessage>().last()
        val fitted = budgeter.fitPrompt(constrainedRequest, firstFit)

        assertEquals(ConversationContextPolicy.SUMMARY_SOURCE.name, firstFitSummary.metadata[ContextPolicyMetadata.KEY])
        assertTrue(fitted.instructions.none { it === firstFitSummary })
        assertTrue(fitted.instructions.filterIsInstance<AssistantMessage>().any { it.text == "prior answer" })
    }

    @Test
    fun `retains the initiating user turn whenever its assistant answer is retained`() {
        val messages =
            listOf(
                Message("system", "rules"),
                Message("user", "prior question"),
        ...
        val request = ChatRequestContext(messages, contextWindow = ContextWindow(configuredLimit = 500))

        val result = budgeter.fit(request, messages)

        assertTrue(result.messages.any { it.role == "assistant" && it.content == "prior answer" })
        assertTrue(result.messages.any { it.role == "user" && it.content == "prior question" })
    }
